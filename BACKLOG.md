...
- ❓ **batch zod-issue error reporting** (2026-05-17) — `/api/accounts` and `/api/transactions` collapse multi-field validation failures to `issues[0]?.message`. M2-5 TradeSheet now does client-side per-field validation via zodResolver, so the server-issue mapping is less urgent; revisit when DIV/SPLIT or CSV-import paths land (they bypass the RHF schema). Defer to M3.
- ❓ **ensureDefaultProfile on every render** (2026-05-17) — `app/(app)/s/[symbol]/page.tsx` calls it for the TradeSheet accounts pre-fetch. Function is idempotent (SELECT then conditional INSERT) but the SELECT happens every render. Trivial today; M3 should split into a read-only `getCurrentProfileId()` path and call bootstrap only when truly empty.
- ❓ **TanStack QueryClient + §7.5 invalidation cascade** (2026-05-17) — Deferred through M2 (M2-2 / M2-4 / M2-5 all use plain fetch + router.refresh). M3 must (a) set up QueryClient + Provider in `app/(app)/layout.tsx`, (b) write `lib/api/queryKeys.ts` with profile-scoped key factories enforcing contract #4, (c) build `useProfileScopedQuery` + `useProfileMutation` wrappers that throw at dev time, (d) write the central `invalidateAfterTxnMutation(qc, profileId, txn)` helper per INTERACTION_SPEC §7.5, (e) migrate AccountCreateForm + TradeSheet to use useMutation.
- ❓ **TradeSheet success toast** (2026-05-17) — Successful submit dismisses sheet + router.refresh() but gives no explicit success feedback. INTERACTION_SPEC §11 lists a "Saved" toast for transactions.create. Add sonner toast when the "Toast + Undo for trade delete" BACKLOG item lands (M2 same sprint) — they'll share the Toaster mount.
